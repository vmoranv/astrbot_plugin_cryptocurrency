...
| 命令 (中文别名) | 功能描述 | 示例 |
| --- | --- | --- |
| `/crypto` (`/查币价`) | 查询币种的详细市场数据。 | `/crypto btc` |
| `/trending` (`/热门币种`) | 获取 CoinGecko 热门币种。 | `/trending` |
| `/global` (`/市场概览`) | 获取全球加密市场概览。 | `/global` |
| `/chart` (`/价格图`) | 获取币种7日价格走势图。 | `/chart solana` |
| `/cry_history` (`/历史价格`) | 获取币种历史价格摘要 (默认7天)。 | `/cry_history eth,30` |
| `/gainerslosers` (`/涨跌榜`) | 显示24小时市场涨跌幅榜。 | `/gainerslosers` |
| `/categories` (`/所有分类`) | 列出所有币种分类。 | `/categories` |
| `/category` (`/分类查询`) | 获取特定分类下的币种。 | `/category stablecoins` |
| `/exchange` (`/交易所信息`) | 获取交易所信息。 | `/exchange binance` |
| `/cry_tickers` (`/交易对`) | 获取币种的交易对信息。 | `/cry_tickers btc,binance` |
| `/networks` (`/网络列表`) | 列出支持的区块链网络。 | `/networks` |
| `/config_currencies` (`/目标币种`) | 显示当前配置的目标加密货币。 | `/config_currencies` |
...
| 命令 (中文别名) | 功能描述 | 示例 |
| --- | --- | --- |
| `/cry_fight` (`/投资模拟`) | 使用指定的起始资金开始一个新的投资模拟。 | `/投资模拟 10000` |
| `/cry_fight finish` | 结束当前模拟，平掉所有仓位并进行详细结算。 | `/投资模拟 finish` |
| `/cry_fight_status` (`/持仓状态`) | 查看当前投资组合的详细状态和盈亏。 | `/持仓状态` |
